...
void MainWindow::on_pushButton_Dibujar_2_clicked()
{
    coordenadas = new QGraphicsScene(this);
    QPen pen;
    pen.setWidth(2);
    ui->graphicsView->setScene(coordenadas);
    for (int z = 0; z < neuronas_ingre; z++)
    {
        QGraphicsTextItem *paren = coordenadas->addText("(               )");
        paren->setPos(repetidos_x[z]-5, repetidos_y[z]);
        QGraphicsTextItem *text = coordenadas->addText(QString ::number(repetidos_x[z]));
        text->setPos(repetidos_x[z], repetidos_y[z]);
        QGraphicsTextItem *text1 = coordenadas->addText(",");
        text1->setPos(repetidos_x[z]+23, repetidos_y[z]);
        QGraphicsTextItem *text2 = coordenadas->addText(QString ::number(repetidos_y[z]));
        text2->setPos(repetidos_x[z]+25, repetidos_y[z]);
    }
}

void Lista::mas_cercano(int contador)
{
    int comparar[50];
    int guarx[50];
    int guary[50];
    int corr = contador;
    int sin_cambio = contador;
    while(contador < neuronas_ingre-1)
    {
        cout << "Contador: "<<contador<<endl;
        cout<< "repetidos_x1: "<<repetidos_x[corr]<<" - repetidos_x2: "<<repetidos_x[contador + 1]<<endl;
        guarx[contador] = repetidos_x[corr] - repetidos_x[contador + 1];
        if(guarx[contador] < 0)
        {
           guarx[contador] = repetidos_x[contador + 1] - repetidos_x[corr];
        }
        cout << "Guardarx: ["<<contador<<"]: "<<guarx[contador]<<endl;
        guary[contador] = repetidos_y[corr] - repetidos_y[contador + 1];
        if(guary[contador] < 0)
        {
           guary[contador] = repetidos_y[contador + 1] - repetidos_y[corr];
        }
        cout << "Guardary: ["<<contador<<"]: "<<guary[contador]<<endl;
        comparar[contador] = guarx[contador] + guary[contador];
        cout << "Comparar: "<<comparar[contador]<<endl;
        contador++;
    }
    while(corr < neuronas_ingre-2)
    {
        cout << "Comparar ["<<corr<<"]: "<<comparar[corr]<<" > "<<"Comparar ["<<corr+1<<"]: "<<comparar[corr+1]<<endl;
        if(comparar[corr] > comparar[corr+1])
        {
            cout << "Comparar entro"<<endl;
            cout << "Comparar ["<<corr<<"]: "<<comparar[corr]<<" > "<<"Comparar ["<<corr+1<<"]: "<<comparar[corr+1]<<endl;
            mejor_x[sin_cambio] = repetidos_x[corr+2];
            mejor_y[sin_cambio] = repetidos_y[corr+2];
        }
        else if(contador+1 != neuronas_ingre)
        {
            comparar[corr+1] = comparar[corr];
        }
        corr++;
    }
    cout <<"Adentro mejor x["<<sin_cambio<<"]: "<<mejor_x[sin_cambio]<<endl;
    cout <<"Adentro mejor y["<<sin_cambio<<"]: "<<mejor_y[sin_cambio]<<endl;
    if (mejor_x[sin_cambio] == 0 && mejor_y[sin_cambio] == 0)
    {
        mejor_x[sin_cambio] = repetidos_x[sin_cambio + 1];
        mejor_y[sin_cambio] = repetidos_y[sin_cambio + 1];
    }
}

void Lista::volver_llenar()
{
    for(int i = 0; i < neuronas_ingre; i++)
    {
        repetidos_suma[i] = repetidos_x[i] + repetidos_y[i];
    }
    ...
    for (int gap = neuronas_ingre / 2; gap > 0; gap /= 2)
    {
        for (int i = gap; i < neuronas_ingre; i += 1)
        {
            ...
            int temp_g = repetidos_green[i];
            int temp_b = repetidos_blue[i];
            int temp_s = repetidos_suma[i];
            int j;
            for (j = i; j >= gap && repetidos_suma[j - gap] > temp_s; j -= gap)
            {
                repetidos_id[j] = repetidos_id[j - gap];
                repetidos_voltaje[j] = repetidos_voltaje[j - gap];
                repetidos_x[j] = repetidos_x[j - gap];
...
void MainWindow::on_pushButton_Dibujar_3_clicked()
{
    coordenadas = new QGraphicsScene(this);
    QPen pen;
    ui->graphicsView->setScene(coordenadas);
    //mi_lista.volver_llenar();
    for (int z = 0; z < neuronas_ingre; z++)
    {
        QColor color(repetidos_red[z],repetidos_green[z],repetidos_blue[z]);
        pen.setColor(color);
        pen.setWidth(3);
        coordenadas->addEllipse(repetidos_x[z],repetidos_y[z],3,3,pen);
        if (z < neuronas_ingre-1)
        {
            mi_lista.mas_cercano(z);
            pen.setWidth(1);
            cout <<"Fuera mejor x: "<<mejor_x[z]<<endl;
            cout <<"Fuera mejor y: "<<mejor_y[z]<<endl;
            coordenadas->addLine(repetidos_x[z],repetidos_y[z],repetidos_x[z+1],repetidos_y[z+1],pen);
        }
    }
}
